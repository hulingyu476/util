    ...
    for (const auto& ro_product_prop : RO_PRODUCT_PROPS) {
        ...
        for (const auto& source : Split(ro_product_props_source_order, ",")) {
            ...
            if (!target_prop_val.empty()) {
                if (base_prop.compare("ro.product.model") == 0 && target_prop_val.compare("M22") == 0) {
                    std::string platform = GetProperty("persist.hht.Platforms", EMPTY);
                    LOG(INFO) << "platform " << platform << ", screen_size " << screen_size;
                    if (!platform.empty() && platform.compare("M22") == 0) {
                        if (!screen_size.empty() && screen_size.compare("S65") == 0) {
                            target_prop_val = "SLUNRK";
                        } else if (!screen_size.empty() && screen_size.compare("S86") == 0) {
                            target_prop_val = "SBLNRK";
                        } else {    // S75
                            target_prop_val = "STUNRK";
                        }
                    }
                }
...
static void ProcessKernelCmdline() {
    ...
    ImportKernelCmdline([&](const std::string& key, const std::string& value) {
        ...
        } else if (StartsWith(key, "screen_size")) {
            screen_size = value;
        }
    });
